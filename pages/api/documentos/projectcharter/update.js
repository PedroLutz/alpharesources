...
export default async (req, res) => {
  try {
    ...
    if (req.method === 'PUT') {
      ...
      if (!id) {
        return res.status(400).json({ error: 'O ID do ProjectCharter é obrigatório para a atualização.' });
      }
      ...
      const updatedData = await ProjectCharter.findByIdAndUpdate(id, updateFields, { new: true });
    ...
    console.error('Erro ao atualizar o ProjectCharter', error);
    res.status(500).json({ error: 'Erro ao atualizar o ProjectCharter' });
  }
};
