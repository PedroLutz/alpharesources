...
export default async (req, res) => {
  try {
    ...
    if (req.method === 'POST') {
      ...
      const newData = new Funcao(requestBodyObject);

      await newData.save();

      res.status(201).json({ message: 'Funcao cadastrado com sucesso!' });
    } else {
      res.status(405).json({ error: 'Método não permitido' });
    }
  } catch (error) {
    console.error('Erro ao cadastrar o Funcao', error);
    res.status(500).json({ error: 'Erro ao cadastrar o Funcao' });
  }
};
