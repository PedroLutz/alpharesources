...
export default async (req, res) => {
  try {
    ...
    if (req.method === 'PUT') {
      ...
      if (!id) {
        return res.status(400).json({ error: 'O ID do Plano é obrigatório para a atualização.' });
      }
      ...
      const updatedData = await Plano.findByIdAndUpdate(id, updateFields, { new: true });
