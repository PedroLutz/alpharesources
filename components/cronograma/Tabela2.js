...
const Tabela = () => {
  ...
  const filteredCronogramas = cronogramas.filter((item) => {
    ...
    const itemMatch = item.item.toLowerCase().includes(filtroItem.toLowerCase());
  ...
  const formatDataForGantt = (date) => {
    return `Date(${date.getUTCFullYear()}, ${date.getUTCMonth()}, ${date.getUTCDate()})`;
  };
  ...
  const createGanttData = (cronogramas) => {
    ...
    cronogramas.forEach((item) => {
      if (item.plano) {
        ...
        const startDate = item.inicio;
        const endDate = item.termino;
